...
- Armored is serialized archetype 5, Normal tier, with three armor stages and no health/status UI.
...
- Persistent colors: blue (0.22, 0.42, 0.72), teal (0.10, 0.80, 0.75),
  amber (1.00, 0.65, 0.12), rust (0.72, 0.25, 0.18). Inspector-configurable in the spawn profile.
...
[Unity EditMode results](Armored/editmode-results.xml): **147 passed, 0 failed, 0 skipped**.
...
eligibility, prefab physics, sampled animation, navigation, boss rules and twelve-level scene wiring.
...
- Baked Armored render entities have valid sample blobs and finite skin matrices. Gameplay-camera
  images were inspected: [opening](Armored/opening.png), [cracked hit](Armored/cracked-impact.png).
- [Four persistent stage colors](Armored/stage-colors.png) were inspected on arranged baked enemies
  after transient feedback had cleared. Both skinned renderers reported the exact configured color
  for every stage. This capture uses a centered inspection camera.
...
| System | ms/update |
| --- | ---: |
| Chase and separation | 0.048 |
| Navigation | 0.024 |
| Movement | 0.007 |
| Collision, settled contacts | 0.064 |
| Hit-history cleanup | 0.004 |
| Aim assistance | 0.367 |
| Readability | 0.155 |
| Sampled animation | 0.124 |
...
Rebuild content using "Crowd Punch/Enemies/Rebuild Armored Prefab" and
