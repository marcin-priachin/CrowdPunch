...
5. `PunchDetectionSystem` finds living active, launched, or recovering enemies inside the punch volume, clears existing linear and angular velocity when a player punch replaces an active launch, starts a fresh `Launched` sequence with the current punch data, and enables impulse and damage requests (COMBAT-014).
