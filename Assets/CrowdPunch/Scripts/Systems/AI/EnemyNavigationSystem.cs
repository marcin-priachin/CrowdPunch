...
namespace CrowdPunch.Systems.AI
{
    ...
    public partial struct EnemyNavigationSystem : ISystem
    {
        ...
        private NativeReference<int> expanded;
        private NativeReference<int> searchCursor;
        ...
        public void OnCreate(ref SystemState state)
        {
            ...
            expanded = new NativeReference<int>(Allocator.Persistent);
            searchCursor = new NativeReference<int>(Allocator.Persistent);
        }
        public void OnDestroy(ref SystemState state) { storage.Dispose();queue.Dispose();expanded.Dispose();searchCursor.Dispose(); }
        public void OnUpdate(ref SystemState system)
        {
            using var marker=Marker.Auto();long started=System.Diagnostics.Stopwatch.GetTimestamp();
                    ...
                    .WithAll<Enemy>().WithOptions(EntityQueryOptions.IgnoreComponentEnabledState).WithEntityAccess())
            {
                ...
                if(n.Initialized==0)
                {
                    ...
                    n.ProgressPosition=position;n.LastPosition=position;n.ProgressAt=now;
                ...
                bool goalChanged=math.distancesq(n.RequestedGoal,goal.Destination.xz)>settings.DestinationThreshold*settings.DestinationThreshold || n.Kind!=goal.Kind;
            ...
            // Shared search expansion is Burst compiled. Native scratch survives budget exhaustion.
            var work=new SearchWork {Storage=storage,Grid=grid.Data,Budget=settings.GlobalBudget,Limit=settings.SearchLimit,Expanded=expanded,Cursor=searchCursor};
            work.Run();storage.Cursor=searchCursor.Value;
            diagnostics.Expanded=expanded.Value;diagnostics.Searching=0;
            for(int i=0;i<storage.Slots.Length;i++)
            {
            ...
            diagnostics.Milliseconds=settings.Diagnostics!=0?(System.Diagnostics.Stopwatch.GetTimestamp()-started)*1000d/System.Diagnostics.Stopwatch.Frequency:0;
        ...
        { n.Pending=0;n.TravelState=NavigationTravelState.Failed;n.NextRequestAt=now+s.FailureDelay;n.NextDirectAt=now+s.FailureDelay;if(limited)d.LimitReached++;else d.Failures++; }
            ...
            NavigationRuntimeSettings settings,float braking)
        {
            ...
            if(!NavigationGeometry.Segment(ref g,position,position+direction.xz*probe,radius))
            {
                ...
                probe=math.min(probe,settings.WaypointArrival);
                if(!NavigationGeometry.Segment(ref g,position,position+direction.xz*probe,radius))return default;
                speed=math.min(speed,math.sqrt(2*math.max(.1f,braking)*probe));
            }
            return new DesiredMovement {Direction=direction,Speed=speed};
        }
        [BurstCompile]
        private struct SearchWork : IJob
        {
            public NavigationSearchStorage Storage;
            public BlobAssetReference<NavigationGridBlob> Grid;
            public int Budget,Limit;
            public NativeReference<int> Expanded,Cursor;
            public void Execute(){Expanded.Value=Storage.ExpandBudget(ref Grid.Value,Budget,Limit);Cursor.Value=Storage.Cursor;}
        }
    }
}
