...
namespace CrowdPunch.Bakers
{
    public sealed class EnemyAnimationBaker : Baker<EnemyAnimationAuthoring>
    {
        public override void Bake(EnemyAnimationAuthoring authoring)
        {
            ...
            if (reader.ReadInt32() != 0x43504131)
                throw new InvalidDataException("Unsupported enemy animation samples.");
            ...
            var durations = builder.Allocate(ref data.Durations, 9);
            for (int i = 0; i < 9; i++) durations[i] = reader.ReadSingle();
            var matrices = builder.Allocate(ref data.Matrices, 9 * frames * bones);
