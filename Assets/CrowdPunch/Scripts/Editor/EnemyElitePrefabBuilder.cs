...
{
    public static class EnemyElitePrefabBuilder
    {
        private const string ModelPath = "Assets/CrowdPunch/Models/UltimateMonsters/Big/Yeti.fbx";
        private const string PrefabPath = "Assets/CrowdPunch/Prefabs/EliteEnemy.prefab";
        private const string ControllerPath = "Assets/CrowdPunch/Animation/EnemyElite.controller";
        private const string SettingsPath = "Assets/CrowdPunch/Data/Settings/Enemies/EliteEnemySpawnSettings.asset";
        private const string MaterialFolder = "Assets/CrowdPunch/Materials/Enemies/Elite";
        ...
        [MenuItem("Crowd Punch/Enemies/Rebuild Elite Prefab")]
        public static void Rebuild()
        {
            EnsureFolder("Assets/CrowdPunch/Materials/Enemies", "Elite");
            ConfigureAnimationImport();

            GameObject model = AssetDatabase.LoadAssetAtPath<GameObject>(ModelPath);
            if (model == null) throw new InvalidOperationException($"Missing Elite model: {ModelPath}");
            AnimationClip idle = FindClip("Idle");
            AnimationClip walk = FindClip("Walk");
            AnimationClip death = FindClip("Death");
            AnimationClip punch = FindClip("Punch");
            AnimationClip hitReact = FindClip("HitReact");
            AnimatorController controller = BuildController(idle, walk, death, punch, hitReact);

            GameObject root = new GameObject("EliteEnemy") { layer = 7 };
            try
            {
                ...
                if (renderers.Length == 0)
                    throw new InvalidOperationException("Yeti.fbx contains no skinned renderers.");
                ...
                for (int index = 0; index < renderers.Length; index++)
                {
                    ...
                    string samplesPath = $"Assets/CrowdPunch/Animation/EnemyEliteMovement_{index:00}_{SafeName(renderer.name)}.bytes";
                    ...
                    authoring.Profile = EnemyAnimationProfile.Elite;
            ...
            Debug.Log($"Created {PrefabPath} from Yeti.fbx and assigned it to {SettingsPath}.");
        }
        ...
        private static AnimationClip FindClip(string name)
        {
            ...
            throw new InvalidOperationException($"Yeti.fbx has no animation named '{name}'. Available: {available}");
        }
        ...
        private static Material[] BuildMaterials(Material[] sources)
        {
            ...
            for (int index = 0; index < sources.Length; index++)
            {
                ...
                string name = string.IsNullOrWhiteSpace(source?.name) ? $"Yeti_{index}" : source.name;
        ...
        private static void FitVisualToEnemy(Transform enemy, Transform visual, SkinnedMeshRenderer[] renderers)
        {
            ...
            if (largest <= Mathf.Epsilon) throw new InvalidOperationException("Yeti renderers have empty bounds.");
            visual.localScale *= 3.5f / largest;
        ...
        private static void AssignSpawnSettings()
        {
            ...
            if (settings == null || prefab == null) throw new InvalidOperationException("Could not load Elite settings or prefab.");
