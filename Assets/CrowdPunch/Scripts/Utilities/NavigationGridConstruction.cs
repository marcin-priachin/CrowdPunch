            ...
            NativeArray<NavigationRectangle> rectangles,Allocator allocator)
        {
            ...
            for(int cls=0;cls<3;cls++)
            {
                ...
                for(int cell=0;cell<count;cell++)
                {
                    ...
                    while(head<tail) { int current=queue[head++];byte mask=data.Edges[cls*count+current];
                        for(int d=0;d<8;d++)if((mask&(1<<d))!=0) { int n=NavigationGeometry.Neighbor(ref data,current,d);
